...
SIMILARITY_THRESHOLD = 0.4  # Default threshold, will be optimized
    ...
    for person_name in tqdm(persons_list, desc="Processing persons"):
        ...
        for test_idx, test_image_path in enumerate(person_images):
            try:
                ...
                # Extract features
                test_embedding, proc_time = extract_resnet_features(test_face)
                ...
                for train_idx, train_image_path in enumerate(person_images):
                    if train_idx != test_idx:  # Skip the test image
                        ...
                        if train_face is not None and train_face.size > 0:
                            train_embedding, _ = extract_resnet_features(train_face)
                    ...
                    if other_person != person_name:
                        ...
                        for other_image_path in other_images:
                            ...
                            if other_face is not None and other_face.size > 0:
                                other_embedding, _ = extract_resnet_features(other_face)
...
def calculate_metrics(results, optimal_thresholds):
    ...
    for metric, threshold in optimal_thresholds.items():
        ...
        # For demonstration, applying threshold to similarity scores
        # In practice, this should be adjusted based on the specific metric
...
def validate_face_recognition_model():
    print("Starting face recognition model validation...")
...
if __name__ == "__main__":
    validate_face_recognition_model()